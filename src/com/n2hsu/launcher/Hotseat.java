...
public class Hotseat extends FrameLayout {
	...
	void resetLayout() {
		...
		if (!AppsCustomizePagedView.DISABLE_ALL_APPS) {
			...
			// Note: We do this to ensure that the hotseat is always laid out in
			// the orientation of
			// the hotseat in order regardless of which orientation they were
			// added
	...
	void addAllAppsFolder(IconCache iconCache, ArrayList<AppInfo> allApps,
			ArrayList<ComponentName> onWorkspace, Launcher launcher, Workspace workspace) {
		if (AppsCustomizePagedView.DISABLE_ALL_APPS) {
			...
			LauncherModel.addItemToDatabase(launcher, fi, fi.container, fi.screenId, fi.cellX, fi.cellY,
					false);
			FolderIcon folder = FolderIcon
					.fromXml(R.layout.folder_icon, launcher, getLayout(), fi, iconCache);
	...
	void addAppsToAllAppsFolder(ArrayList<AppInfo> apps) {
		if (AppsCustomizePagedView.DISABLE_ALL_APPS) {
			View v = mContent.getChildAt(getCellXFromOrder(mAllAppsButtonRank),
					getCellYFromOrder(mAllAppsButtonRank));
			FolderIcon fi = null;
