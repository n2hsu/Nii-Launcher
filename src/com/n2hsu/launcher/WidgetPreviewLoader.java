...
public class WidgetPreviewLoader {
	...
	public Bitmap getPreview(final Object o) {
		...
		synchronized (mUnusedBitmaps) {
			...
			while ((unusedBitmap == null || !unusedBitmap.isMutable()
					|| unusedBitmap.getWidth() != mPreviewBitmapWidth || unusedBitmap.getHeight() != mPreviewBitmapHeight)
		...
		if (preview != null) {
			...
			// write to db on a thread pool... this can be done lazily and
	...
	static class CacheDb extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase database) {
			database.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (" + COLUMN_NAME
					+ " TEXT NOT NULL, " + COLUMN_SIZE + " TEXT NOT NULL, " + COLUMN_PREVIEW_BITMAP
					+ " BLOB NOT NULL, " + "PRIMARY KEY (" + COLUMN_NAME + ", " + COLUMN_SIZE + ") " + ");");
		}

		@Override
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			if (oldVersion != newVersion) {
				// Delete all the records; they'll be repopulated as this is a
				// cache
	...
	private static String getObjectName(Object o) {
		...
		if (o instanceof AppWidgetProviderInfo) {
			...
			sb.append(new ComponentName(info.activityInfo.packageName, info.activityInfo.name)
					.flattenToString());
			output = sb.toString();
			sb.setLength(0);
		}
	...
	public static void removePackageFromDb(final CacheDb cacheDb, final String packageName) {
		...
		new AsyncTask<Void, Void, Void>() {
			public Void doInBackground(Void... args) {
				...
				db.delete(CacheDb.TABLE_NAME, CacheDb.COLUMN_NAME + " LIKE ? OR " + CacheDb.COLUMN_NAME
						+ " LIKE ?", // SELECT query
						new String[] { WIDGET_PREFIX + packageName + "/%",
								SHORTCUT_PREFIX + packageName + "/%" } // args
																		// to
																		// SELECT
																		// query
				);
	...
	public static void removeItemFromDb(final CacheDb cacheDb, final String objectName) {
		new AsyncTask<Void, Void, Void>() {
			public Void doInBackground(Void... args) {
				SQLiteDatabase db = cacheDb.getWritableDatabase();
				db.delete(CacheDb.TABLE_NAME, CacheDb.COLUMN_NAME + " = ? ", // SELECT
																				// query
	...
	private Bitmap readFromDb(String name, Bitmap b) {
		...
		Cursor result = db.query(CacheDb.TABLE_NAME, new String[] { CacheDb.COLUMN_PREVIEW_BITMAP }, // cols
																										// to
																										// return
	...
	public Bitmap generatePreview(Object info, Bitmap preview) {
		...
		if (info instanceof AppWidgetProviderInfo) {
			...
			return generateShortcutPreview((ResolveInfo) info, mPreviewBitmapWidth, mPreviewBitmapHeight,
					preview);
		}
	}
	...
	public Bitmap generateWidgetPreview(AppWidgetProviderInfo info, Bitmap preview) {
		...
		return generateWidgetPreview(info.provider, info.previewImage, info.icon, cellSpans[0], cellSpans[1],
	...
	public Bitmap generateWidgetPreview(ComponentName provider, int previewImage, int iconId, int cellHSpan,
			int cellVSpan, int maxPreviewWidth, int maxPreviewHeight, Bitmap preview, int[] preScaledWidthOut) {
		...
		if (widgetPreviewExists) {
			...
			BitmapDrawable previewDrawable = (BitmapDrawable) mContext.getResources().getDrawable(
					R.drawable.widget_tile);
			...
			try {
				...
				if (icon != null) {
					renderDrawableToBitmap(icon, defaultPreview, hoffset, yoffset,
		...
		// If a bitmap is passed in, we use it; otherwise, we create a bitmap of
		// the right size
		if (preview == null) {
	...
	private Bitmap generateShortcutPreview(ResolveInfo info, int maxWidth, int maxHeight, Bitmap preview) {
		...
		int paddingTop = mContext.getResources()
				.getDimensionPixelOffset(R.dimen.shortcut_preview_padding_top);
		int paddingLeft = mContext.getResources().getDimensionPixelOffset(
		...
		c.setBitmap(preview);
		// Draw a desaturated/scaled version of the icon in the background as a
		// watermark
