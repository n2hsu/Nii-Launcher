...
public class DragLayer extends FrameLayout implements ViewGroup.OnHierarchyChangeListener {
	...
	public boolean onRequestSendAccessibilityEvent(View child, AccessibilityEvent event) {
		...
		if (currentFolder != null) {
			...
			// Skip propagating onRequestSendAccessibilityEvent all for other
			// children
	 ...
	 *            Whether or not to account for the scroll of the root
	...
	public void animateViewIntoPosition(DragView dragView, final int[] pos, float alpha, float scaleX,
			float scaleY, int animationEndStyle, Runnable onFinishRunnable, int duration) {
		Rect r = new Rect();
	...
	public void animateViewIntoPosition(DragView dragView, final View child, int duration,
		...
		if (child instanceof TextView) {
			...
			// The child may be scaled (always about the center of the view) so
			// to account for it,
			// we have to offset the position by the scaled size. Once we do
			// that, we can center
		...
		animateViewIntoPosition(dragView, fromX, fromY, toX, toY, 1, 1, 1, scale, scale, onCompleteRunnable,
	...
	public void animateViewIntoPosition(final DragView view, final int fromX, final int fromY, final int toX,
			final int toY, float finalAlpha, float initScaleX, float initScaleY, float finalScaleX,
			float finalScaleY, Runnable onCompleteRunnable, int animationEndStyle, int duration,
			View anchorView) {
	...
	public void animateView(final DragView view, final Rect from, final Rect to, final float finalAlpha,
		...
		AnimatorUpdateListener updateCb = new AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				float alphaPercent = alphaInterpolator == null ? percent : alphaInterpolator
						.getInterpolation(percent);
				float motionPercent = motionInterpolator == null ? percent : motionInterpolator
						.getInterpolation(percent);
				...
				int xPos = x - mDropView.getScrollX()
						+ (mAnchorView != null ? (mAnchorViewInitialScrollX - mAnchorView.getScrollX()) : 0);
	...
	protected int getChildDrawingOrder(int childCount, int i) {
		...
		// go about this.
		if (true || LauncherAppState.isScreenLandscape(getContext()) || mWorkspaceIndex == -1
				|| mQsbIndex == -1 || mLauncher.getWorkspace().isDrawingBackgroundGradient()) {
			return i;
		}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mInScrollArea && !LauncherAppState.getInstance().isScreenLarge()) {
			...
			if (leftPage != null && leftPage.getIsDragOverlapping()) {
				mLeftHoverDrawable.setBounds(0, childRect.top, mLeftHoverDrawable.getIntrinsicWidth(),
						childRect.bottom);
				mLeftHoverDrawable.draw(canvas);
			} else if (rightPage != null && rightPage.getIsDragOverlapping()) {
				mRightHoverDrawable.setBounds(width - mRightHoverDrawable.getIntrinsicWidth(), childRect.top,
						width, childRect.bottom);
