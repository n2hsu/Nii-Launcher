...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
	...
	/**
	 * The restore page will be set in place of the current page at the next
	 * (likely first) layout.
	 */
	...
	// While layout transitions are occurring, a child's position may stray from
	// its baseline
	// position. This method returns the magnitude of this stray at any given
	// time.
	...
	int getNearestHoverOverPageIndex() {
		if (mDragView != null) {
			int dragX = (int) (mDragView.getLeft() + (mDragView.getMeasuredWidth() / 2) + mDragView
					.getTranslationX());
			getOverviewModePages(mTempVisiblePagesRange);
		...
		case MotionEvent.ACTION_UP:
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (!mFreeScroll) {
					...
					mScroller.fling(initialScrollX, getScrollY(), vX, 0, Integer.MIN_VALUE,
	...
	protected void loadAssociatedPages(int page, boolean immediateAndOnly) {
		if (mContentIsRefreshable) {
			...
			if (page < count) {
				...
				if (DEBUG)
					Log.d(TAG, "loadAssociatedPages: " + lowerPageBound + "/" + upperPageBound);
	...
	protected void invalidatePageData(int currentPage, boolean immediateAndOnly) {
		...
		if (isPageMoving()) {
			// If the page is moving, then snap it to the final position to
			// ensure we don't get
			// stuck between pages
			snapToDestination();
		}
	...
	protected String getCurrentPageDescription() {
		return String.format(getContext().getString(R.string.default_scroll_format), getNextPage() + 1,
				getChildCount());
	}
