...
public class FocusHelper {
	...
	/**
	 * Handles key events in a AppsCustomize tab between the last tab view and
	 * the shop button.
	 */
	static boolean handleAppsCustomizeTabKeyEvent(View v, int keyCode, KeyEvent e) {
		...
		case KeyEvent.KEYCODE_DPAD_DOWN:
			if (handleKeyEvent) {
				// Select the content view (down is handled by the tab key
				// handler otherwise)
				if (v == shop) {
	...
	/**
	 * Returns the Viewgroup containing page contents for the page at the index
	 * specified.
	 */
	...
	/**
	 * Handles key events in a PageViewExtendedLayout containing
	 * PagedViewWidgets.
	 */
	static boolean handlePagedViewGridLayoutWidgetKeyEvent(PagedViewWidget w, int keyCode, KeyEvent e) {
		...
		case KeyEvent.KEYCODE_DPAD_LEFT:
			if (handleKeyEvent) {
				// Select the previous widget or the last widget on the previous
				// page
				if (widgetIndex > 0) {
		...
		case KeyEvent.KEYCODE_DPAD_UP:
			if (handleKeyEvent) {
				// Select the closest icon in the previous row, otherwise select
				// the tab bar
				if (y > 0) {
		...
		case KeyEvent.KEYCODE_DPAD_DOWN:
			if (handleKeyEvent) {
				// Select the closest icon in the previous row, otherwise do
				// nothing
				if (y < (cellCountY - 1)) {
		...
		case KeyEvent.KEYCODE_PAGE_UP:
			if (handleKeyEvent) {
				// Select the first item on the previous page, or the first item
				// on this page
		...
		case KeyEvent.KEYCODE_PAGE_DOWN:
			if (handleKeyEvent) {
				// Select the first item on the next page, or the last item on
				// this page
	...
	static boolean handleAppsCustomizeKeyEvent(View v, int keyCode, KeyEvent e) {
		...
		final boolean handleKeyEvent = (action != KeyEvent.ACTION_UP);
		ViewGroup newParent = null;
		// Side pages do not always load synchronously, so check before focusing
		// child siblings
		// willy-nilly
		View child = null;
		boolean wasHandled = false;
		...
		case KeyEvent.KEYCODE_DPAD_LEFT:
			if (handleKeyEvent) {
				// Select the previous icon or the last icon on the previous
				// page
				if (iconIndex > 0) {
		...
		case KeyEvent.KEYCODE_PAGE_UP:
			if (handleKeyEvent) {
				// Select the first icon on the previous page, or the first icon
				// on this page
				// if there is no previous page
				if (pageIndex > 0) {
					newParent = getAppsCustomizePage(container, pageIndex - 1);
		...
		case KeyEvent.KEYCODE_PAGE_DOWN:
			if (handleKeyEvent) {
				// Select the first icon on the next page, or the last icon on
				// this page
				// if there is no next page
				if (pageIndex < (pageCount - 1)) {
					newParent = getAppsCustomizePage(container, pageIndex + 1);
	...
	static boolean handleTabKeyEvent(AccessibleTabView v, int keyCode, KeyEvent e) {
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			if (handleKeyEvent) {
				// Select the next tab, or if the last tab has a focus right id,
				// select that
	...
	static boolean handleHotseatButtonKeyEvent(View v, int keyCode, KeyEvent e, int orientation) {
		...
		// orientations, even though the hotseat is on the side in landscape
		// mode. This
		// is to ensure that accessibility consistency is maintained across
		// rotations.
		...
		case KeyEvent.KEYCODE_DPAD_LEFT:
			if (handleKeyEvent) {
				// Select the previous button, otherwise snap to the previous
				// page
				if (buttonIndex > 0) {
	...
	/**
	 * Private helper method to get the CellLayoutChildren given a CellLayout
	 * index.
	 */
	...
	/**
	 * Private helper method to sort all the CellLayout children in order of
	 * their (x,y) spatially from top left to bottom right.
	 */
	...
	/**
	 * Private helper method to find the index of the next BubbleTextView or
	 * FolderIcon in the direction delta.
	 * 
	 * @param delta
	...
	/**
	 * Private helper method to find the next closest BubbleTextView or
	 * FolderIcon in the direction delta on the next line.
	 * 
	 * @param delta
	 *            either -1 or 1 depending on the line and direction we want to
	 *            search
	 */
	...
	static boolean handleIconKeyEvent(View v, int keyCode, KeyEvent e) {
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			if (handleKeyEvent) {
				// Select the next icon or the first icon on the next page if
				// possible
				View newIcon = getIconInDirection(layout, parent, v, 1);
				if (newIcon != null) {
					newIcon.requestFocus();
		...
		case KeyEvent.KEYCODE_DPAD_UP:
			if (handleKeyEvent) {
				// Select the closest icon in the previous line, otherwise
				// select the tab bar
		...
		case KeyEvent.KEYCODE_DPAD_DOWN:
			if (handleKeyEvent) {
				// Select the closest icon in the next line, otherwise select
				// the button bar
				View newIcon = getClosestIconOnLine(layout, parent, v, 1);
				if (newIcon != null) {
					newIcon.requestFocus();
