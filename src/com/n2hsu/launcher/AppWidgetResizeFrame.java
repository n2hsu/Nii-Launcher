...
public class AppWidgetResizeFrame extends FrameLayout {
	...
	public AppWidgetResizeFrame(Context context, LauncherAppWidgetHostView widgetView, CellLayout cellLayout,
		...
		lp = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, Gravity.LEFT
		...
		lp = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, Gravity.CENTER_HORIZONTAL
				| Gravity.TOP);
		...
		// When we create the resize frame, we first mark all cells as
		// unoccupied. The appropriate
		// cells (same if not resized, or different) will be marked as occupied
		// when the resize
		// frame is dismissed.
	...
	public boolean beginResizeIfPointInRegion(int x, int y) {
		...
		boolean anyBordersActive = mLeftBorderActive || mRightBorderActive || mTopBorderActive
				|| mBottomBorderActive;
	...
	/**
	 * Here we bound the deltas such that the frame cannot be stretched beyond
	 * the extents of the CellLayout, and such that the frame's borders can't
	 * cross.
	 */
	...
	/**
	 * Based on the deltas, we resize the frame, and, if needed, we resize the
	 * widget.
	 */
	...
	/**
	 * Based on the current deltas, we determine if and how to resize the
	 * widget.
	 */
	private void resizeWidgetIfNeeded(boolean onDismiss) {
		...
		// For each border, we bound the resizing based on the minimum width,
		// and the maximum
		// expandability.
		if (mLeftBorderActive) {
		...
		// Update the widget's dimensions and position according to the deltas
		// computed above
		...
		// We always want the final commit to match the feedback, so we make
		// sure to use the
		...
		if (mCellLayout.createAreaForResize(cellX, cellY, spanX, spanY, mWidgetView, mDirectionVector,
				onDismiss)) {
			lp.tmpCellX = cellX;
			lp.tmpCellY = cellY;
			lp.cellHSpan = spanX;
	...
	/**
	 * This is the final step of the resize. Here we save the new widget size
	 * and position to LauncherModel and animate the resize frame.
	 */
	...
	public void snapToWidget(boolean animate) {
		...
		// We need to make sure the frame's touchable regions lie fully within
		// the bounds of the
		// DragLayer. We allow the actual handles to be clipped, but we shift
		// the touch regions
		// down accordingly to provide a proper touch target.
		if (newY < 0) {
			// In this case we shift the touch region down to start at the top
			// of the DragLayer
		...
		if (newY + newHeight > mDragLayer.getHeight()) {
			// In this case we shift the touch region up to end at the bottom of
			// the DragLayer
