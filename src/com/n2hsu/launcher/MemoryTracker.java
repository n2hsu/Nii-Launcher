...
public class MemoryTracker extends Service {
	...
	public static final String ACTION_START_TRACKING = "com.n2hsu.launcher.action.START_TRACKING";
	...
	public static void startTrackingMe(Context context, String name) {
		context.startService(new Intent(context, MemoryTracker.class)
				.setAction(MemoryTracker.ACTION_START_TRACKING).putExtra("pid", android.os.Process.myPid())
				.putExtra("name", name));
	}
	...
	void update() {
		synchronized (mLock) {
			...
			for (int i = 0; i < dinfos.length; i++) {
				...
				// Log.v(TAG, "update: pid " + pid + " pss=" + info.currentPss +
				// " uss=" + info.currentUss);
