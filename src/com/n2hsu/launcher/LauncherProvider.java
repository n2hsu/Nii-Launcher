...
import com.n2hsu.launcher.config.ProviderConfig;
...
public class LauncherProvider extends ContentProvider {
	...
	/**
	 * @param Should
	 *            we load the old db for upgrade? first run only.
	 */
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE favorites (" + "_id INTEGER PRIMARY KEY," + "title TEXT,"
					...
					+ "iconType INTEGER," + "iconPackage TEXT," + "iconResource TEXT," + "icon BLOB,"
					+ "uri TEXT," + "displayMode INTEGER," + "appWidgetProvider TEXT,"
			...
			// final
			// upgrade code (ie. to fix workspace screen indices -> ids, etc.),
			// so
			// set that flag too.
		...
		private boolean convertDatabase(SQLiteDatabase db, Uri uri, ContentValuesCallback cb,
				boolean deleteRows) {
			if (LOGD)
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (version < 13) {
				// With the new shrink-wrapped and re-orderable workspaces, it
				// makes sense
				// to persist workspace screens and their relative order.
				mMaxScreenId = 0;

				// This will never happen in the wild, but when we switch to
				// using workspace
		...
		private boolean updateContactsShortcuts(SQLiteDatabase db) {
			...
			try {
				...
				c = db.query(TABLE_FAVORITES, new String[] { Favorites._ID, Favorites.INTENT }, selectWhere,
						null, null, null, null);
				if (c == null)
					return false;
		...
		// call it when we
		// call the constructor from the worker thread; however, this doesn't
		// extend until after the
		// constructor is called, and we only pass a reference to
		// LauncherProvider to LauncherApp
		// after that point
		public long generateNewScreenId() {
		...
		private long initializeMaxScreenId(SQLiteDatabase db) {
			Cursor c = db.rawQuery("SELECT MAX(" + LauncherSettings.WorkspaceScreens._ID + ") FROM "
			...
			try {
				...
				while (((type = parser.next()) != XmlPullParser.END_TAG || parser.getDepth() > depth)
					...
					if (LOGD) {
						...
						Log.v(TAG, String.format(("%" + (2 * (depth + 1)) + "s<%s%s c=%d s=%s x=%s y=%s>"),
								"", name, (something == null ? "" : (" \"" + something + "\"")), container,
								screen, x, y));
					}
		...
		private boolean addAppWidget(SQLiteDatabase db, ContentValues values, ComponentName cn, int spanX,
				int spanY, Bundle extras) {
