...
public class DeleteDropTarget extends ButtonDropTarget {
	...
	protected void onFinishInflate() {
		...
		// uninstall drawable
		// and is initially set to the remove drawable, as set in the layout
		// xml.
	...
	public void onDragStart(DragSource source, Object info, int dragAction) {
		...
		// If we are dragging an application from AppsCustomize, only show the
		// control if we can
		// delete the app (it was downloaded), and rename the string to
		// "uninstall" in such a case.
	...
	private void completeDrop(DragObject d) {
		...
		if (isAllAppsApplication(d.dragSource, item)) {
			// Uninstall the application if it is being dragged from
			// AppsCustomize
			...
			if (shortcut.intent != null && shortcut.intent.getComponent() != null) {
				...
				int flags = AppInfo.initFlags(ShortcutInfo.getPackageInfo(getContext(),
				...
				if (mWaitingForUninstall) {
					final Runnable checkIfUninstallWasSuccess = new Runnable() {
						...
						public void run() {
							...
							List<ResolveInfo> activities = AllAppsList.findActivitiesForPackage(getContext(),
									packageName);
	...
	private AnimatorUpdateListener createFlingToTrashAnimatorListener(final DragLayer dragLayer,
		...
		// Calculate how far along the velocity vector we should put the
	...
	/**
	 * Creates an animation from the current drag view along its current
	 * velocity vector. For this animation, the alpha runs for a fixed duration
	 * and we update the position progressively.
	 */
	...
	private AnimatorUpdateListener createFlingAlongVectorAnimatorListener(final DragLayer dragLayer,
			DragObject d, PointF vel, final long startTime, final int duration, ViewConfiguration config) {
		final Rect from = new Rect();
		dragLayer.getViewRectRelativeToSelf(d.dragView, from);
	...
	public void onFlingToDelete(final DragObject d, int x, int y, PointF vel) {
			...
			public float getInterpolation(float t) {
				if (mCount < 0) {
					...
					mOffset = Math.min(0.5f,
							(float) (AnimationUtils.currentAnimationTimeMillis() - startTime) / duration);
					mCount++;
				}
		...
		Runnable onAnimationEndRunnable = new Runnable() {
			...
			public void run() {
				...
				// AppsCustomizePagedView to clean up
				// itself, otherwise, complete the drop to initiate the deletion
				// process
				if (!isAllApps) {
					mLauncher.exitSpringLoadedDragMode();
					completeDrop(d);
