...
public class SavedWallpaperImages extends BaseAdapter implements ListAdapter {
	...
	public static class SavedWallpaperTile extends WallpaperPickerActivity.WallpaperTileInfo {
		...
		public void onClick(WallpaperPickerActivity a) {
			...
			v.setTileSource(new BitmapRegionTileSource(a, file.getAbsolutePath(), 1024, rotation), null);
			v.moveToLeft();
	...
	public void loadThumbnailsAndImageIdList() {
		...
		Cursor result = db.query(ImageDb.TABLE_NAME, new String[] { ImageDb.COLUMN_ID,
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		return WallpaperPickerActivity.createImageTileView(mLayoutInflater, position, convertView, parent,
				thumbDrawable);
	}
	...
	private Pair<String, String> getImageFilenames(int id) {
		SQLiteDatabase db = mDb.getReadableDatabase();
		Cursor result = db.query(ImageDb.TABLE_NAME, new String[] { ImageDb.COLUMN_IMAGE_THUMBNAIL_FILENAME,
	...
	public void deleteImage(int id) {
		...
		db.delete(ImageDb.TABLE_NAME, ImageDb.COLUMN_ID + " = ?", // SELECT
																	// query
				new String[] { Integer.toString(id) // args to SELECT query
				});
	}
	...
	public void writeImage(Bitmap thumbnail, byte[] imageBytes) {
		try {
			...
			FileOutputStream imageFileStream = mContext.openFileOutput(imageFile.getName(),
					Context.MODE_PRIVATE);
			imageFileStream.write(imageBytes);
			imageFileStream.close();

			File thumbFile = File.createTempFile("wallpaperthumb", "", mContext.getFilesDir());
			FileOutputStream thumbFileStream = mContext.openFileOutput(thumbFile.getName(),
					Context.MODE_PRIVATE);
	...
	static class ImageDb extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase database) {
			database.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (" + COLUMN_ID
					+ " INTEGER NOT NULL, " + COLUMN_IMAGE_THUMBNAIL_FILENAME + " TEXT NOT NULL, "
					+ COLUMN_IMAGE_FILENAME + " TEXT NOT NULL, " + "PRIMARY KEY (" + COLUMN_ID + " ASC) "
					+ ");");
		}
