...
public class CellLayout extends ViewGroup {
	...
	public CellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		for (int i = 0; i < mDragOutlineAnims.length; i++) {
			final InterruptibleInOutAnimator anim = new InterruptibleInOutAnimator(this, duration,
				...
				public void onAnimationUpdate(ValueAnimator animation) {
					...
					if (outline == null) {
						...
						if (debug) {
							...
							Log.d(TAG,
									"anim " + thisIndex + " update: " + val + ", isStopped "
											+ anim.isStopped());
						}
	...
	private void invalidateBubbleTextView(BubbleTextView icon) {
		...
		invalidate(icon.getLeft() + getPaddingLeft() - padding, icon.getTop() + getPaddingTop() - padding,
				icon.getRight() + getPaddingLeft() + padding, icon.getBottom() + getPaddingTop() + padding);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mPressedOrFocusedIcon != null) {
			...
			if (b != null) {
				...
				canvas.drawBitmap(b, mPressedOrFocusedIcon.getLeft() + left - padding,
						mPressedOrFocusedIcon.getTop() + top - padding, null);
			}
		}
	 ...
	 */
	static void getMetrics(Rect metrics, int paddedMeasureWidth, int paddedMeasureHeight, int countX,
			int countY) {
		LauncherAppState app = LauncherAppState.getInstance();
		DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		// Expand the background drawing bounds by the padding baked into the
		// background drawable
		Rect padding = new Rect();
		mNormalBackground.getPadding(padding);
		mBackgroundRect.set(-padding.left, -padding.top, w + padding.right, h + padding.bottom);

		mForegroundRect.set(mForegroundPadding, mForegroundPadding, w - mForegroundPadding, h
				- mForegroundPadding);
	}
	...
	void visualizeDropLocation(View v, Bitmap dragOutline, int originX, int originY, int cellX, int cellY,
			int spanX, int spanY, boolean resize, Point dragOffset, Rect dragRegion) {
		...
		if (cellX != oldDragCellX || cellY != oldDragCellY) {
			...
			if (v != null && dragOffset == null) {
				...
				if (dragOffset != null && dragRegion != null) {
					...
					// outline offset
					left += dragOffset.x
							+ ((mCellWidth * spanX) + ((spanX - 1) * mWidthGap) - dragRegion.width()) / 2;
	...
	int[] findNearestVacantArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
		...
		return findNearestVacantArea(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, null, result,
				resultSpan);
	}
	...
	int[] findNearestArea(int pixelX, int pixelY, int spanX, int spanY, View ignoreView,
		...
		return findNearestArea(pixelX, pixelY, spanX, spanY, spanX, spanY, ignoreView, ignoreOccupied,
				result, null, mOccupied);
	}
	...
	int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
			View ignoreView, boolean ignoreOccupied, int[] result, int[] resultSpan, boolean[][] occupied) {
		...
		for (int y = 0; y < countY - (minSpanY - 1); y++) {
			inner: for (int x = 0; x < countX - (minSpanX - 1); x++) {
				...
				double distance = Math
						.sqrt(Math.pow(cellXY[0] - pixelX, 2) + Math.pow(cellXY[1] - pixelY, 2));
	...
	private boolean addViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
		...
		findNearestArea(boundingRect.left, boundingRect.top, boundingRect.width(), boundingRect.height(),
				direction, mTmpOccupied, blockOccupied, mTempLocation);
	...
	ItemConfiguration simpleSwap(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
		...
		if (!success) {
			...
			if (spanX > minSpanX && (minSpanY == spanY || decX)) {
				return simpleSwap(pixelX, pixelY, minSpanX, minSpanY, spanX - 1, spanY, direction, dragView,
						false, solution);
			} else if (spanY > minSpanY) {
				return simpleSwap(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY - 1, direction, dragView,
						true, solution);
			}
	...
	private void animateItemsToSolution(ItemConfiguration solution, View dragView, boolean commitDragView) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			CellAndSpan c = solution.map.get(child);
			if (c != null) {
				animateChildToPosition(child, c.x, c.y, REORDER_ANIMATION_DURATION, 0, DESTRUCTIVE_REORDER,
						false);
	...
	private void beginOrAdjustHintAnimations(ItemConfiguration solution, View dragView, int delay) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			LayoutParams lp = (LayoutParams) child.getLayoutParams();
			if (c != null) {
				ReorderHintAnimation rha = new ReorderHintAnimation(child, lp.cellX, lp.cellY, c.x, c.y,
						c.spanX, c.spanY);
				rha.animate();
			}
		}
	...
	class ReorderHintAnimation {
		...
		public ReorderHintAnimation(View child, int cellX0, int cellY0, int cellX1, int cellY1, int spanX,
				int spanY) {
	...
	boolean createAreaForResize(int cellX, int cellY, int spanX, int spanY, View dragView, int[] direction,
		...
		// layout
		ItemConfiguration swapSolution = simpleSwap(pixelXY[0], pixelXY[1], spanX, spanY, spanX, spanY,
				direction, dragView, true, new ItemConfiguration());
	...
	int[] createArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY, View dragView,
		...
		ItemConfiguration swapSolution = simpleSwap(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY,
				mDirectionVector, dragView, true, new ItemConfiguration());

		// We attempt the approach which doesn't shuffle views at all
		ItemConfiguration noShuffleSolution = findConfigurationNoShuffle(pixelX, pixelY, minSpanX, minSpanY,
				spanX, spanY, dragView, new ItemConfiguration());
	...
	int[] findNearestVacantArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
		...
		return findNearestArea(pixelX, pixelY, minSpanX, minSpanY, spanX, spanY, ignoreView, true, result,
				resultSpan, mOccupied);
	}
	 ...
	 */
	boolean findCellForSpanThatIntersectsIgnoring(int[] cellXY, int spanX, int spanY, int intersectX,
			int intersectY, View ignoreView, boolean occupied[][]) {
		// mark space take by ignoreView as available (method checks if
		// ignoreView is null)
		markCellsAsUnoccupiedForView(ignoreView, occupied);
	...
	public static int[] rectToCell(int width, int height, int[] result) {
		...
		DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
		Rect padding = grid
				.getWorkspacePadding(grid.isLandscape ? CellLayout.LANDSCAPE : CellLayout.PORTRAIT);

		// Always assume we're working with the smallest span to make sure we
		// reserve enough space in both orientations.
		int parentWidth = grid.calculateCellWidth(grid.widthPx - padding.left - padding.right,
				(int) grid.numColumns);
		int parentHeight = grid.calculateCellHeight(grid.heightPx - padding.top - padding.bottom,
				(int) grid.numRows);
