...
public abstract class SmoothPagedView extends PagedView {
	...
	public SmoothPagedView(Context context, AttributeSet attrs, int defStyle) {
		...
		// This means that we'll take care of updating the scroll parameter
		// ourselves (we do it
		// in computeScroll), we only do this in the OVERSHOOT_MODE, ie. on
		// phones
	...
	public void computeScroll() {
		if (mScrollMode == X_LARGE_MODE) {
			...
			if (!scrollComputed && mTouchState == TOUCH_STATE_SCROLLING) {
				...
				// Keep generating points as long as we're more than 1px away
				// from the target
				if (dx > 1.f || dx < -1.f) {
					invalidate();
				}
