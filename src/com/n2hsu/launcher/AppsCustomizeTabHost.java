...
public class AppsCustomizeTabHost extends TabHost implements LauncherTransitionable,
	...
	public void onTabChanged(String tabId) {
		...
		// We post a runnable here because there is a delay while the first page
		// is loading and
		...
		post(new Runnable() {
			...
			public void run() {
				...
				// Take the visible pages and re-parent them temporarily to
				// mAnimatorBuffer
				...
				// We want the pages to be rendered in exactly the same way as
				// they were when
				// their parent was mAppsCustomizePane -- so set the scroll on
				// mAnimationBuffer
				// to be exactly the same as mAppsCustomizePane, and below, set
				// the left/top
				...
				// add the pages to mAnimationBuffer in reverse order to match
				// that behavior
	...
	public void onLauncherTransitionEnd(Launcher l, boolean animated, boolean toWorkspace) {
		...
		if (!toWorkspace) {
			...
			// Going from Workspace -> All Apps
			// NOTE: We should do this at the end since we check visibility
			// state in some of the
