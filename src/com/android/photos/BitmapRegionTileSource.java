...
public class BitmapRegionTileSource implements TiledImageRenderer.TileSource {
	...
	private BitmapRegionTileSource(Resources res, Context context, String path, Uri uri, int resId,
			int previewSize, int rotation) {
		...
		if (previewSize != 0) {
			...
			// Although this is the same size as the Bitmap that is likely
			// already
			// loaded, the lifecycle is different and interactions are on a
			// different
			...
			if (preview.getWidth() <= GL_SIZE_LIMIT && preview.getHeight() <= GL_SIZE_LIMIT) {
				...
				Log.w(TAG, String.format("Failed to create preview of apropriate size! "
						+ " in: %dx%d, out: %dx%d", mWidth, mHeight, preview.getWidth(), preview.getHeight()));
			}
		}
	}
	...
	/**
	 * Note that the returned bitmap may have a long edge that's longer than the
	 * targetSize, but it will always be less than 2x the targetSize
	 */
	private Bitmap decodePreview(Resources res, Context context, String file, Uri uri, int resId,
			int targetSize) {
		...
		// or didn't support the specified inSampleSize (some decoders only do
		// powers of 2)
