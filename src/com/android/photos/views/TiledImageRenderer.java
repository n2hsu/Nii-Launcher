...
public class TiledImageRenderer {
	...
	public static interface TileSource {
		...
		/**
		 * Return a Preview image if available. This will be used as the base
		 * layer if higher res tiles are not yet available
		 */
		 ...
		 * 0) - (width, height) and (x, y) - (x + tileSize, y + tileSize). If in
		 * extending the region, we found some part of the region is outside the
		 * image, those pixels are filled with black.
		 * 
	...
	private class Tile extends UploadedTexture {
		...
		public String toString() {
			return String
					.format("tile(%s, %s, %s / %s)", mX / mTileSize, mY / mTileSize, mLevel, mLevelCount);
		}
	}
