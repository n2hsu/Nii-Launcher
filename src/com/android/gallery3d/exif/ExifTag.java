...
public class ExifTag {
	...
	public boolean setValue(String value) {
		...
		byte[] finalBuf = buf;
		if (buf.length > 0) {
			finalBuf = (buf[buf.length - 1] == 0 || mDataType == TYPE_UNDEFINED) ? buf : Arrays.copyOf(buf,
					buf.length + 1);
	...
	protected void getBytes(byte[] buf, int offset, int length) {
		...
		System.arraycopy(mValue, 0, buf, offset, (length > mComponentCountActual) ? mComponentCountActual
				: length);
	}
